...
class draw2Dataset(BaseDataset):
    """ LasHeR dataset for RGB-T tracking.
    ...
    def __init__(self, split):
        ...
        # Split can be test, val, or ltrval (a validation split consisting of videos from the official train set)
        if split == 'testingset' or split == 'val':
            self.base_path = os.path.join(self.env_settings.draw2_path, split)
        else:
            self.base_path = os.path.join(self.env_settings.draw2_path, 'train')
