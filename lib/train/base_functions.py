...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val",
                        "COCO17", "VID", "TRACKINGNET", "LasHeR_train",  "LasHeR_test", "VTUAV", "vtuav_test"]
        if name == "LASOT":
        ...
        if name == "vtuav_test":
            datasets.append(VTUAV(settings.env.vtuav_test_dir, split='val_st', image_loader=image_loader))
...
def get_optimizer_scheduler(net, cfg):
    ...
    if train_cls:
        ...
        param_dicts = [
            {"params": [p for n, p in net.named_parameters() if "smmt" in n and p.requires_grad]},
            {
                "params": [p for n, p in net.named_parameters() if "smmt" not in n and p.requires_grad],
